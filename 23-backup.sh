...
if [ $# -lt 2 ] #  Check if at Least One Argument Is Supplied
then
    USAGE
fi

if [ ! -d $SOURCE_DIR ] # check the directory is exit or not
then
    echo -e "$R Source Directory $SOURCE_DIR does not exist. Please check $N"
    exit 1
fi

if [ ! -d $DEST_DIR ]  # check the directory is exit or not
then
...
if [ ! -z "$FILES" ]  # check the file is exit or not
then
